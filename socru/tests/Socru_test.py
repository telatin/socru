...
class TestSocru(unittest.TestCase):
    ...
    def test_socru_valid(self):
        g = Socru(Options([os.path.join(data_dir, 'test.fa')], 'Salmonella_enterica', None, 1000,1000,1, 'output_file', False, 'novel', 'newfrag.fa', None, 'blast', 'output_plot.png', 'directions'))
        g.run()
    ...
    def test_operon_wrapped_around_ends(self):
        ...
        g = Socru(Options([os.path.join(data_dir, 'wrapped.fa')], 'Salmonella_enterica', None, 1000,1000,1, 'output_file', False, 'novel', 'newfrag.fa', None, 'blast', 'output_plot.png', 'directions'))
        g.run()
        self.assertTrue(os.path.exists('output_file'))
        self.assertTrue(os.path.exists('blast'))
