...
class TestSocruCreate(unittest.TestCase):
    ...
    def test_compressed_input(self):
        ...
        g = SocruCreate(Options('test_output', os.path.join(data_dir, 'test.fa.gz'), 1, os.path.join(data_dir, 'dnaA.fa.gz'), os.path.join(data_dir, 'dif.fa.gz'), None, fragment_order = None))
        g.run()
    ...
    def test_socru_create(self):
        ...
        g = SocruCreate(Options('test_output', os.path.join(data_dir, 'test.fa'), 1, os.path.join(data_dir, 'dnaA.fa.gz'), os.path.join(data_dir, 'dif.fa.gz'), None, fragment_order = None))
        g.run()
        self.assertTrue(os.path.exists('test_output/1.fa'))
        self.assertTrue(os.path.exists('test_output/2.fa'))
        self.assertTrue(os.path.exists('test_output/3.fa'))
    ...
    def test_socru_create_set_fragment_order(self):
        ...
        g = SocruCreate(Options('test_output_reorder', os.path.join(data_dir, 'test.fa'), 1,os.path.join(data_dir, 'dnaA.fa.gz'), os.path.join(data_dir, 'dif.fa.gz'), None, fragment_order = "1-4-3'-2"))
        g.run()
